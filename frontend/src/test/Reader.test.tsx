...
describe("Reader", () => {
  it("prompts to select when nothing is open", () => {
    render(<Reader item={null} onMarkUnread={vi.fn()} onRetry={vi.fn()} onBack={vi.fn()} onSave={vi.fn()} />);
  ...
  it("renders the article, metadata, and a working Mark unread", async () => {
    ...
    render(<Reader item={full({ read_state: "read" })} onMarkUnread={onMarkUnread} onRetry={vi.fn()} onBack={vi.fn()} onSave={vi.fn()} />);
  ...
  it("shows the analyzing state while enrichment is pending", () => {
    render(
      <Reader
        ...
        onMarkUnread={vi.fn()}
        onRetry={vi.fn()} onBack={vi.fn()} onSave={vi.fn()}
      />
    );
  ...
  it("shows the failed state with a retry that fires onRetry", async () => {
    ...
    render(
      <Reader
        ...
        onMarkUnread={vi.fn()}
        onRetry={onRetry} onBack={vi.fn()} onSave={vi.fn()}
      />
    );
  ...
  it("flags a partial capture", () => {
    render(<Reader item={full({ extraction_status: "partial" })} onMarkUnread={vi.fn()} onRetry={vi.fn()} onBack={vi.fn()} onSave={vi.fn()} />);
  ...
  it("offers a back control that fires onBack (mobile navigation)", async () => {
    const onBack = vi.fn();
    render(<Reader item={full()} onMarkUnread={vi.fn()} onRetry={vi.fn()} onBack={onBack} onSave={vi.fn()} />);
  ...
  it("shows Save to library only for feed items and fires onSave", async () => {
    ...
    const { rerender } = render(
      <Reader item={full({ lane: "saved" })} onMarkUnread={vi.fn()} onRetry={vi.fn()} onBack={vi.fn()} onSave={onSave} />
    );
    expect(screen.queryByRole("button", { name: /save to library/i })).not.toBeInTheDocument();

    rerender(
      <Reader item={full({ id: 3, lane: "feed" })} onMarkUnread={vi.fn()} onRetry={vi.fn()} onBack={vi.fn()} onSave={onSave} />
    );
